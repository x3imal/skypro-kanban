...
export default function Calendar({
    ...
    const [viewYear, setViewYear] = useState(
        (selectedInit || today).getFullYear()
    );
    const [viewMonth, setViewMonth] = useState(
        (selectedInit || today).getMonth()
    );
    ...
    return (
        <Wrap>
            ...
            <Block>
                ...
                <Content>
                    <Days>
                        {WEEKDAYS_SHORT.map((d) => <Day key={d}>{d}</Day>)}
                    </Days>

                    <Cells>
                        {grid.map((d) => {
                            const inThisMonth = d.getMonth() === viewMonth;
                            const isWeekend = [0,6].includes(d.getDay()); // 0=вс, 6=сб
                ...
                <Period>
                    <p>
                        {showHint ? (
                            <>Выберите срок исполнения <span className="date-control">{selected ? formatRu(selected) : ""}</span></>
                        ) : (
                            <>Срок исполнения: <span className="date-control">{selected ? formatRu(selected) : ""}</span></>
                        )}
                    </p>
                </Period>
            </Block>
